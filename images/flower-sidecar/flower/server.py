...
def main(grid,context):
    caps = {k.split('.',1)[1]:v for k,v in context.run_config.items() if k.startswith('capabilities')}
    log(INFO, f"Requested capabilities: {caps}")
    content = RecordDict({"query_request":ConfigRecord(caps)})
    ndarrays = [
    ...
    for _id in node_ids:
        ...
        log(INFO, "Sending query to %s", _id)
    ...
    allowed_ids = []
    log(INFO, "Received %d replies", len(replies))
    for reply in replies:
        _id = reply.metadata.src_node_id
        eligible = reply.content["query_response"]["eligible"]
        if eligible:
            allowed_ids.append(_id)
        log(INFO, "Node %d replied: %s", _id, eligible)
    num_eligible_nodes = len(allowed_ids)
    if num_eligible_nodes == 0:
        log(INFO, "No eligible nodes answered the query. Aborting orchestration.")
        return
    ...
    strategy = FedAvg(
        ...
        weighted_by_key="num-examples",
        min_available_nodes=num_eligible_nodes,
        min_train_nodes=num_eligible_nodes,
        min_evaluate_nodes=num_eligible_nodes
    )
    ...
    log(INFO, "Finished learning, now logging")
